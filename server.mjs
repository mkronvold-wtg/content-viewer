...
async function refreshRepoContent(state) {
    const git = await updateDisposableClone(state);
    state.index = null;
    state.indexPromise = null;
    const index = await ensureIndex(state);
    return { repo: index.repo, git, total: index.docs.length, indexedAt: index.indexedAt };
}

async function refreshContent(appState) {
    const results = [];
    for (const state of appState.repoStates.values()) {
        results.push(await refreshRepoContent(state));
    }
    return results;
}
...
async function startStandaloneServer() {
    ...
    console.log(`serving markdown from ${state.repos.map((repo) => `${repo.slug}:${repo.path}`).join(", ")}`);

    for (const repoState of state.repoStates.values()) {
        void ensureIndex(repoState).then((index) => {
            console.log(`indexed ${index.docs.length} markdown files for ${index.repo.slug}`);
        }).catch((error) => {
    ...
    if (refreshIntervalMs) {
        setInterval(() => {
            void refreshContent(state).then((result) => {
                console.log(`refreshed index: ${result.map((entry) => `${entry.repo.slug}:${entry.total}`).join(", ")}`);
            }).catch((error) => {
                console.error(error instanceof Error ? error.message : String(error));
            });
